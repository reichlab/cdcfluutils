...
make_predictions_plots <- function(
  ...
  colnames(predictions) <- tolower(colnames(predictions))
  ...
  for(region in unique(data$region)) {
    ...
    if(regional) {
      ...
      if(!is.na(point_pred$season_week)) {
    ...
    reduced_preds <- predictions[predictions$location == preds_region &
        predictions$target %in% paste0(1:4, " wk ahead") &
    ...
    p_1wk <- my_plot_weekahead(reduced_preds, region = preds_region, wk = 1, ilimax=13, years = 2019:2020) + ggtitle(paste(preds_region, ": 1 wk ahead")) + ylim(0,1) + geom_vline(xintercept = recent_obs)
    p_2wk <- my_plot_weekahead(reduced_preds, region = preds_region, wk = 2, ilimax=13, years = 2019:2020) + ylim(0,1) + geom_vline(xintercept = recent_obs)
    p_3wk <- my_plot_weekahead(reduced_preds, region = preds_region, wk = 3, ilimax=13, years = 2019:2020) + ylim(0,1) + geom_vline(xintercept = recent_obs)
    p_4wk <- my_plot_weekahead(reduced_preds, region = preds_region, wk = 4, ilimax=13, years = 2019:2020) + ylim(0,1) + geom_vline(xintercept = recent_obs)
